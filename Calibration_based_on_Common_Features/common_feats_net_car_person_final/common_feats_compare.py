...
############### Lei ###################################
import sys
sys.path.append("./common_feats_net_car_person_final/") # add search path: sys.path.append("../../")
#######################################################
import os
import numpy as np
...
from common_feats_net_car_person_final.utils.utils import cvt2Color, preprocess_input, resize_image,resize_radar_data,resize_with_only_padding
from common_feats_net_car_person_final.nets.CSPdarknet53 import Mish
...
def load_common_feats_model(full_model_path = "common_feats_net_car_person_final/model_data/final_model.h5"):
    ############### Lei ###################################
    import sys
    ...
    #sys.path.insert(0, "./common_feats_net_car_person_final/")
    #######################################################
    #full_model_path = "common_feats_net_car_person_final/model_data/final_model.h5"
    # Load the model
    model = tf.keras.models.load_model(full_model_path, custom_objects={'Mish': Mish})
    return model

def common_feats_find(images, rad_complexs,model=None):
    if model is None:
        ############### Lei ###################################
        import sys
        sys.path.append("./common_feats_net_car_person_final/") # add search path: sys.path.append("../../")
    ...
    img_idx_file    = 'common_feats_net_car_person_final/model_data/img_idx.txt'
    ...
    ##########################  Test  #########################################
    test_dataset   = prepare_img_rad_data(images, rad_complexs, new_size_rad=input_shape_rad,new_size_img=input_shape_img)
...
if __name__ == "__main__":
    triple_idx_file   = 'model_data/triple_idx_sijie-car-lei.txt' #'./triple_idx_lei-car-lei.txt' #'./triple_idx_2class.txt' #
    ...
    predictions = common_feats_find(images, rad_complexs)
