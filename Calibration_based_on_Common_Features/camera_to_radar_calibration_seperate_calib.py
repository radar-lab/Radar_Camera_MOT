...
def extract_arrays(radar_points, transformed_points, x_thr, y_thr):
    mask = (radar_points[:, 0] - transformed_points[:, 0, 0] < x_thr) & \
           (radar_points[:, 1] - transformed_points[:, 0, 1] < y_thr)
    
    extracted_radar_points = radar_points[mask]
    extracted_transformed_points = transformed_points[mask]

    return extracted_radar_points, extracted_transformed_points
...
def rad_cam_Homography(radar_points, img_points,validation_radar_points, validation_img_points, 
    ...
    # #RANSAC
    homography_matrix, inliers = cv2.findHomography(img_points, radar_points, method=method, 
    ...
    transformed_points = cv2.perspectiveTransform(img_points.reshape(-1, 1, 2), homography_matrix)    
    # Calculate the reprojection error using homography_matrix
    reprojection_errors = transformed_points - radar_points.reshape(-1, 1, 2)
    ...
    ## Inliers
    inliers_points = img_points[inliers.ravel()==1] #inliers.ravel().astype(bool)
    ...
    ## Validation points
    # Apply the perspective transformation using homography_matrix
    transformed_points_val = cv2.perspectiveTransform(validation_img_points.reshape(-1, 1, 2), homography_matrix)    
    # Calculate the reprojection error using homography_matrix
    reprojection_errors = transformed_points_val - validation_radar_points.reshape(-1, 1, 2)
...
def show_calib_result(radar_points, img_points, inliers, inliers_points, transformed_points, transformed_inliers,validation_img_points,validation_radar_points, transformed_points_val):
    if For_Paper:
        validation_radar_points, transformed_points_val = extract_arrays(validation_radar_points, transformed_points_val,
                                                      x_thr=50, y_thr=50)
    # Plot the transformed_points and radar_points with Connect the corresponding points with lines
    # Create a new figure
    fig = plt.figure(figsize=(8, 6), dpi=300)
    ax = fig.add_subplot(111)
    # Plot the transformed_points and radar_points
    ax.scatter(radar_points[:, 0], radar_points[:, 1], c='blue', label='Radar Points')
    ax.scatter(transformed_points[:, 0, 0], transformed_points[:, 0, 1], c='red', label='Image Points')
    # Connect the corresponding points with lines
    for i in range(len(radar_points)):
        x = [radar_points[i, 0], transformed_points[i, 0, 0]]
        y = [radar_points[i, 1], transformed_points[i, 0, 1]]
    ...
    plt.scatter(transformed_points[:, 0, 0], transformed_points[:, 0, 1], c='red', label='Image Points')
    plt.xlabel('X')
    plt.ylabel('Y')
    ...
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title('Original Image Points')
    # Invert the Y-axis
    plt.gca().invert_yaxis()
    # Show legend in the upper left corner
    plt.legend(loc='upper left')
    ...
    plt.figure(figsize=(8, 6), dpi=300)
    plt.scatter(inliers_points[:, 0], inliers_points[:, 1], c='blue', label='Original Image Points')
    plt.scatter(transformed_inliers[:, 0, 0], transformed_inliers[:, 0, 1], c='red', label='Image Points')
    # Connect the corresponding points with lines
    ...
    # Set plot title and labels
    plt.title('Image-Original Image Inlier Correspondences')
    plt.xlabel('X')
    plt.ylabel('Y')
    ...
    # Plot the transformed_inliers and inliers_points for inliers
    plt.figure(figsize=(8, 6), dpi=300)
    radar_inliers = radar_points[inliers.ravel() == 1, :]
    plt.scatter(radar_inliers[:, 0], radar_inliers[:, 1], c='blue', label='Radar Points')
    plt.scatter(transformed_inliers[:, 0, 0], transformed_inliers[:, 0, 1], c='red', label='Image Points')
    # Connect the corresponding points with lines
    for i in range(len(transformed_inliers)):
        x = [radar_inliers[i, 0], transformed_inliers[i, 0, 0]]
        y = [radar_inliers[i, 1], transformed_inliers[i, 0, 1]]
        plt.plot(x, y, color='green', alpha=0.5)
    # Set plot title and labels
    ...
    ax.scatter(radar_points[inliers.ravel() == 1, 0], radar_points[inliers.ravel() == 1, 1], c='none', label='Inliers', facecolors='none', edgecolors='cyan', marker='o', s=50)
    ...
    # Plot the transformed_points_val and validation_radar_points with Connect the corresponding points with lines
    # Create a new figure
    fig = plt.figure(figsize=(8, 6), dpi=300)
    ax = fig.add_subplot(111)
    # Plot the transformed_points_val and validation_img_points
    ax.scatter(validation_radar_points[:, 0], validation_radar_points[:, 1], c='blue', label='Image Points')
    ax.scatter(transformed_points_val[:, 0, 0], transformed_points_val[:, 0, 1], c='red', label='Radar Points')
    # Connect the corresponding points with lines
    for i in range(len(validation_radar_points)):
        x = [validation_radar_points[i, 0], transformed_points_val[i, 0, 0]]
        y = [validation_radar_points[i, 1], transformed_points_val[i, 0, 1]]
        ax.plot(x, y, color='green', alpha=0.5)
...
def up_down_Validation(homography_matrix_up, homography_matrix_down, validation_radar_points, validation_img_points,Y_thr):  
    ...
    # Apply the perspective transformation using homography_matrix
    transformed_points_up = cv2.perspectiveTransform(img_points_up.reshape(-1, 1, 2), homography_matrix_up)    
    # Calculate the reprojection error using homography_matrix
    reprojection_errors = transformed_points_up - radar_points_up.reshape(-1, 1, 2)
    val_reprojection_error = np.mean(np.linalg.norm(reprojection_errors, axis=-1))
    # Output  mean reprojection error
    ...
    # Apply the perspective transformation using homography_matrix
    transformed_points_down = cv2.perspectiveTransform(img_points_down.reshape(-1, 1, 2), homography_matrix_down)    
    # Calculate the reprojection error using homography_matrix
    reprojection_errors = transformed_points_down - radar_points_down.reshape(-1, 1, 2)
    val_reprojection_error = np.mean(np.linalg.norm(reprojection_errors, axis=-1))
    # Output  mean reprojection error
    ...
    if For_Paper:
        radar_points_up, transformed_points_up = extract_arrays(radar_points_up, transformed_points_up,
                                                      x_thr=50, y_thr=50)  
        radar_points_down, transformed_points_down = extract_arrays(radar_points_down, transformed_points_down,
                                                      x_thr=50, y_thr=50) 
    ...
    ax.scatter(radar_points_up[:, 0], radar_points_up[:, 1], c='blue', label='Radar Points')
    ax.scatter(transformed_points_up[:, 0, 0], transformed_points_up[:, 0, 1], c='red', label='Image Points')
    # Connect the corresponding points with lines
    for i in range(len(radar_points_up)):
        x = [radar_points_up[i, 0], transformed_points_up[i, 0, 0]]
        y = [radar_points_up[i, 1], transformed_points_up[i, 0, 1]]
        ax.plot(x, y, color='green', alpha=0.5, label='Up Calib' if i == 0 else "")
        
    # Plot the transformed_points_down and validation_img_points
    ax.scatter(radar_points_down[:, 0], radar_points_down[:, 1], c='blue')
    ax.scatter(transformed_points_down[:, 0, 0], transformed_points_down[:, 0, 1], c='red')
    # Connect the corresponding points with lines
    for i in range(len(radar_points_down)):
        x = [radar_points_down[i, 0], transformed_points_down[i, 0, 0]]
        y = [radar_points_down[i, 1], transformed_points_down[i, 0, 1]]
        ax.plot(x, y, color='orange', alpha=0.5, label='Down Calib' if i == 0 else "")        
...
if __name__ == "__main__":
    ...
    if USE_findHomography:
        ...
        # Save  as an .npy file
        np.save(path+'/homography_matrix_cam2rad.npy', homography_matrix) 
        ...
        homography_matrix, inliers, inliers_points, transformed_points, transformed_inliers, transformed_points_val = \
            rad_cam_Homography(radar_points_up, img_points_up, validation_radar_points, validation_img_points, 
                              method=cv2.RANSAC, ransacReprojThreshold=30.0, maxIters=4000, confidence=0.995)
        # Save  as an .npy file
        np.save(path+'/homography_matrix_cam2rad_up.npy', homography_matrix) 
        # Plot calibration results
        ...
        # Save  as an .npy file
        np.save(path+'/homography_matrix_cam2rad_down.npy', homography_matrix) 
        # Plot calibration results
        ...
        homography_matrix_up   = np.load(path+'/homography_matrix_cam2rad_up.npy', allow_pickle=True)
        homography_matrix_down = np.load(path+'/homography_matrix_cam2rad_down.npy', allow_pickle=True)
