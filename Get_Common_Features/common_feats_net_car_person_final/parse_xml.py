...
from utils.utils import resize_radar_data,resize_with_only_padding
...
if __name__ == '__main__':
    ...
    if DEBUG:
        ...
        for dir_name in subdirectories:
            save_folder = '/RA_objnpy/'
            savepath    = dir_name+save_folder
            all_files = glob.glob(savepath+'*.npy')
            file_names = [os.path.basename(file) for file in all_files]
            sorted_files = sorted(file_names, reverse=False)
            for file in sorted_files:
                # # convert radar complex to real
                rad_complex = np.load(savepath+file,allow_pickle=True)
                RA = getLog(getSumDim(getMagnitude(rad_complex, power_order=2), target_axis=-1), scalar=10, log_10=True)
                plt.imshow(RA, cmap='viridis', origin='lower')# attention --- origin='lower' just for showing
                plt.show()
                rad_real = complexTo2Channels(rad_complex)
    ...
    for dir_name in subdirectories:
        # RA_objnpy folder
        #save_folder = '/test/' ### only for testing if the generated RA npy is correct by showing the RA image
        save_folder = '/RA_objnpy/'
