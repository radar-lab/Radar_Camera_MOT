...
def calculate_val(thresholds, distances, labels, far_target=1e-3, nrof_folds=10):
    ...
    for fold_idx, (train_set, test_set) in enumerate(k_fold.split(indices)):
        ...
        if np.max(far_train)>=far_target:
            # #indices=np.logical_and(far_train>0,far_train<1)
            # u, indices = np.unique(far_train, return_index=True)
            # indices[0]=indices[1]-1            
            # f = interpolate.interp1d(far_train[indices], thresholds[indices], kind='slinear')
